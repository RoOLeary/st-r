...
export default function CookieBanner() {
  const [isDismissed, setIsDismissed] = useState(false); // Start visible

  const dismiss = () => {
    setIsDismissed(true); // Hide on click
  };

  return (
    <div className={`pointer-events-none fixed inset-x-0 bottom-0 px-6 pb-6 ${isDismissed && 'hidden'}`}>
